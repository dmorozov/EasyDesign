...
function generateRecommendations(result: Omit<StyleAuditResult, 'recommendations'>): string[] {
  ...
  const genericFonts = result.typography.filter(
    (t) => t.property === 'font-family' && /arial|helvetica|roboto|inter|system-ui/i.test(t.value),
  );
  ...
  if (pixelSpacing.length > 5) {
    recommendations.push('Convert pixel-based spacing to rem units for better accessibility');
  }
...
async function main(args: string[]): Promise<void> {
  ...
  try {
    ...
    if (options.format === 'summary') {
      ...
      const output = options.pretty ? JSON.stringify(results, null, 2) : JSON.stringify(results);
      console.log(output);
    }
  } catch (error) {
