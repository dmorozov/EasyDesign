...
const TOKEN_PATTERNS: Record<string, RegExp[]> = {
  ...
  typography: [/font/i, /text-/i, /line-height/i, /letter-spacing/i, /word-spacing/i],
  spacing: [/space/i, /gap/i, /margin/i, /padding/i, /inset/i],
  shadow: [/shadow/i, /elevation/i],
  border: [/border/i, /radius/i, /outline/i],
  animation: [/duration/i, /timing/i, /easing/i, /delay/i, /transition/i, /animation/i],
};
...
function formatOutput(tokens: ExtractedTokens, options: ExtractOptions): string {
  switch (options.format) {
    ...
    case 'json':
    default:
      return options.pretty ? JSON.stringify(tokens, null, 2) : JSON.stringify(tokens);
  }
}
