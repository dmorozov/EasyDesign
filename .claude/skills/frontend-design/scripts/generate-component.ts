...
const AESTHETIC_STYLES: Record<Aesthetic, Record<string, string>> = {
  ...
  neumorphism: {
    ...
    focus:
      'focus:outline-none focus:shadow-[inset_4px_4px_8px_#d1d1d1,inset_-4px_-4px_8px_#ffffff]',
...
function getCardTemplate(options: GenerateOptions): { styles: string; component: string } {
  ...
  if (options.styling === 'tailwind') {
    const classes: Record<Aesthetic, string> = {
      ...
      glassmorphism:
        'bg-white/20 backdrop-blur-lg border border-white/30 rounded-2xl p-6 shadow-xl',
      neumorphism:
        'bg-gray-100 rounded-2xl p-6 shadow-[8px_8px_16px_#d1d1d1,-8px_-8px_16px_#ffffff]',
    };
...
function generateReact(
  ...
  template: { styles: string; component: string },
): string {
  const { name } = options;
...
async function main(args: string[]): Promise<void> {
  ...
  const validAesthetics: Aesthetic[] = [
    'minimal',
    'bold',
    'organic',
    'brutalist',
