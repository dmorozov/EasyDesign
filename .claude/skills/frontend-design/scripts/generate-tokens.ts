...
function flattenTokens(obj: Record<string, unknown>, prefix = ''): Record<string, string | number> {
...
function generateTS(spec: TokenSpec): string {
  ...
  lines.push(
    'export const tokens: DesignTokens = ' + JSON.stringify(spec.tokens, null, 2) + ' as const;',
  );
  lines.push('');
  ...
  lines.push(
    'export const flatTokens: Record<string, FlatToken> = ' +
      JSON.stringify(flattenTokens(spec.tokens), null, 2) +
      ';',
  );
  lines.push('');
