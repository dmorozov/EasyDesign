...
function parse(input: string | Buffer | ReadableStream): ParsedData | Promise<ParsedData> {
  // Implementation
}
...
function findById<T extends { id: string }>(items: ReadonlyArray<T>, id: string): T | undefined {
  return items.find((item) => item.id === id);
}

// Use keyof for property access
function pluck<T, K extends keyof T>(items: ReadonlyArray<T>, key: K): Array<T[K]> {
