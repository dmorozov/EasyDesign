...
```typescript
async function processLargeDataset(items: Item[], signal?: AbortSignal): Promise<ProcessedItem[]> {
...
const combinedSignal = linkAbortSignals(timeoutController.signal, userController.signal);
...
async function retryWithBackoff<T>(fn: () => Promise<T>, options: RetryOptions): Promise<T> {
  const { maxAttempts, initialDelayMs, maxDelayMs, backoffMultiplier, signal } = options;
