...
function pipe2<A, B, C>(ab: (a: A) => B, bc: (b: B) => C): (a: A) => C {
  return (a: A) => bc(ab(a));
}

function pipe3<A, B, C, D>(ab: (a: A) => B, bc: (b: B) => C, cd: (c: C) => D): (a: A) => D {
...
// Predicate factory
function createMatcher<T>(key: keyof T, value: T[keyof T]): (item: T) => boolean {
...
```typescript
// Map with index
function mapWithIndex<T, U>(items: ReadonlyArray<T>, fn: (item: T, index: number) => U): U[] {
  return items.map(fn);
}
